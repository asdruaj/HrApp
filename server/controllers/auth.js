...
export const register = async (req, res) => {
  try {
    ...
    if (!tokenFound) return res.status(404).json({ message: 'Enlace Inválido' })
    const tokenData = jwt.verify(linkToken, process.env.JWT_SECRET)

    const filename = req.file?.filename || ''

    const emailFound = await User.findOne({ email })
    if (emailFound) return res.status(403).json({ message: 'Este email ya se encuentra registrado.' })
...
export const login = async (req, res) => {
  try {
    ...
    if (!user) return res.status(400).json({ message: 'Este usuario no existe.' })
    const userObject = user.toObject()

    const isMatch = await bcrypt.compare(password, user.password)
    delete userObject.password

    if (!isMatch) return res.status(400).json({ message: 'Correo o contraseña inválidos.' })
...
export const getTokenData = async (req, res) => {
  try {
    ...
    if (!tokenFound) return res.status(404).json({ message: 'Enlace no válido' })
